...
| `authServerConfigRef` _[api.v1alpha1.ExternalAuthConfigRef](#apiv1alpha1externalauthconfigref)_ | AuthServerConfigRef references an MCPExternalAuthConfig resource that configures<br />an embedded OAuth authorization server (Mode B). When set, the vMCP server acts as<br />an OIDC issuer, drives users through upstream IDPs, and issues ToolHive JWTs.<br />The referenced MCPExternalAuthConfig must have type "embeddedAuthServer" and exist<br />in the same namespace. |  | Optional: \{\} <br /> |
