...
func TestCallbackHandler_TwoUpstreams_StorePendingError_CleansUp(t *testing.T) {
	...
	// Cannot use baseTestSetup because it registers StorePendingAuthorization with
	// AnyTimes() that always succeeds, and gomock does not support overriding.
	// Use storePendingFailTestSetup which wires StorePendingAuthorization to always
	// return an error, while Load/Delete read from the pre-populated state map.
	provider, oauth2Config, stor, storState := storePendingFailTestSetup(t)
