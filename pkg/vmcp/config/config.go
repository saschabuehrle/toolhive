...
type RuntimeConfig struct {
	...
	// AuthServer configures an embedded OAuth authorization server (Mode B).
	// When set, the vMCP server acts as an OIDC issuer, drives users through upstream IDPs,
	// accumulates tokens, and issues ToolHive JWTs. When nil (Mode A), behavior is unchanged.
...
type OIDCConfig struct {
	...
	// Enable when the embedded auth server (Mode B) runs on a loopback address and
